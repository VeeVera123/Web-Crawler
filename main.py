"""
ATS Global Scanner — Main Orchestrator (fully async).
Scans 87,000+ company boards across 20 ATS platforms,
plus 5 remote job boards (RemoteOK, Remotive, Himalayas, Arbeitnow, Jobicy).
"""
import logging
import asyncio

from config import LLM_PROVIDER, LOCATION_PROVIDER
from ats_scrapers import (
    scrape_board, enrich_descriptions, enrich_application_questions, close_client,
)
from job_board_scrapers import scrape_all_job_boards
from classifier import (
    keyword_classify_role, ai_classify_roles,
    keyword_classify_location, ai_classify_locations,
    detect_visa_sponsorship,
)
from supabase_handler import (
    add_jobs_batch, start_scan_report, finish_scan_report,
    get_all_slugs, cleanup_stale_jobs,
    close_client as close_supabase_client,
)

logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s  %(levelname)-8s  %(message)s",
    datefmt="%H:%M:%S",
)
log = logging.getLogger(__name__)

# Per-platform concurrency limits (asyncio semaphores, respect rate limits)
PLATFORM_WORKERS = {
    "greenhouse": 30,
    "lever": 30,
    "ashby": 5,
    "bamboohr": 10,
    "icims": 30,
    "workday": 15,
    "rippling": 8,
    "workable": 10,
    "recruitee": 10,
    "smartrecruiters": 10,
    "teamtailor": 10,
    "breezyhr": 10,
    "applytojob": 10,
    "personio": 10,
    "joincom": 5,
    "taleo": 10,
    "oracle_cloud_hcm": 10,
    "paylocity": 15,
    "hrmdirect": 15,
    "zoho": 5,  # 1.7MB pages, keep low
}


async def load_slugs() -> list[tuple[str, str]]:
    """Load (ats, slug) pairs from Supabase slug_registry."""
    pairs = await get_all_slugs()
    if not pairs:
        log.warning("No slugs found in Supabase slug_registry!")
        log.warning("Run enrich_slugs.py first to populate the registry.")
        return []
    ats_counts: dict[str, int] = {}
    for ats, _ in pairs:
        ats_counts[ats] = ats_counts.get(ats, 0) + 1
    for ats in sorted(ats_counts, key=lambda a: -ats_counts[a]):
        log.info(f"  {ats}: {ats_counts[ats]} companies")
    log.info(f"Total: {len(pairs)} boards across {len(ats_counts)} ATS platforms")
    return pairs


async def scrape_all(boards: list[tuple[str, str]]) -> tuple[list[dict], int, int]:
    """Scrape all boards concurrently, grouped by ATS platform (asyncio).
    Returns (jobs, boards_ok, boards_failed)."""
    all_jobs = []
    total_ok = 0
    total_failed = 0

    by_ats: dict[str, list[str]] = {}
    for ats, slug in boards:
        by_ats.setdefault(ats, []).append(slug)

    async def _scrape_platform(ats: str, slugs: list[str]) -> tuple[int, int, list[dict]]:
        workers = PLATFORM_WORKERS.get(ats, 8)
        sem = asyncio.Semaphore(workers)
        platform_jobs = []
        platform_failed = 0

        async def _do_scrape(slug):
            async with sem:
                return slug, await scrape_board(ats, slug)

        results = await asyncio.gather(*[_do_scrape(s) for s in slugs],
                                       return_exceptions=True)
        for res in results:
            if isinstance(res, Exception):
                platform_failed += 1
                if platform_failed <= 3:
                    log.error(f"  {ats} scrape error: {res}")
                continue
            _slug, jobs = res
            if jobs:
                platform_jobs.extend(jobs)
        return len(slugs) - platform_failed, platform_failed, platform_jobs

    # All platforms run concurrently; each is bounded by its own semaphore
    platform_results = await asyncio.gather(
        *[_scrape_platform(ats, slugs) for ats, slugs in by_ats.items()],
        return_exceptions=True,
    )
    for ats, res in zip(by_ats.keys(), platform_results):
        if isinstance(res, Exception):
            log.error(f"  {ats}: platform error: {res}")
            total_failed += len(by_ats[ats])
            continue
        ok, bad, jobs = res
        all_jobs.extend(jobs)
        total_ok += ok
        total_failed += bad
        log.info(f"  {ats}: {len(jobs)} jobs from {ok} active boards ({bad} failed)")

    log.info(f"Total raw jobs scraped: {len(all_jobs)} ({total_failed} boards failed)")
    return all_jobs, total_ok, total_failed


async def filter_roles(jobs: list[dict]) -> list[dict]:
    """Stage 1+2: Keep only CSM/AM roles."""
    included = []
    unsure = []
    for job in jobs:
        result = keyword_classify_role(job["title"])
        if result == "include":
            included.append(job)
        elif result == "unsure":
            unsure.append(job)
    log.info(f"Role filter: {len(included)} keyword match, {len(unsure)} unsure → sending to AI")
    if unsure:
        unsure_titles = [j["title"] for j in unsure]
        ai_results = await ai_classify_roles(unsure_titles)
        for job in unsure:
            if ai_results.get(job["title"], False):
                included.append(job)
    log.info(f"After role filter: {len(included)} CSM/AM jobs")
    return included


async def filter_locations(jobs: list[dict]) -> tuple[list[dict], list[str]]:
    """Stage 3+4: Keep only global/Africa-eligible jobs."""
    matched = []
    matched_confidences = []
    unsure_jobs = []
    for job in jobs:
        result = keyword_classify_location(job)
        if result == "match":
            job["clearance"] = "regex"
            matched.append(job)
            matched_confidences.append("match")
        elif result == "unsure":
            unsure_jobs.append(job)
    log.info(f"Location filter: {len(matched)} keyword match, {len(unsure_jobs)} unsure → sending to AI")
    if unsure_jobs:
        ai_results = await ai_classify_locations(unsure_jobs)
        for job, label in zip(unsure_jobs, ai_results):
            if label == "match":
                job["clearance"] = LOCATION_PROVIDER["name"]
                matched.append(job)
                matched_confidences.append("match")
            elif label == "uncertain":
                job["clearance"] = LOCATION_PROVIDER["name"]
                matched.append(job)
                matched_confidences.append("uncertain")
    log.info(f"After location filter: {len(matched)} global/Africa jobs")
    return matched, matched_confidences


async def main():
    log.info("=" * 60)
    log.info("ATS GLOBAL SCANNER — starting")
    log.info("=" * 60)
    report_id = await start_scan_report()

    try:
        # 1. Load slugs from Supabase
        log.info("Loading company slugs from Supabase...")
        boards = await load_slugs()
        if not boards:
            log.error("No boards to scrape.")
            if report_id:
                await finish_scan_report(report_id, status="failed")
            return

        # 2. Scrape all ATS boards
        log.info(f"\nScraping {len(boards)} boards across {len(set(a for a, _ in boards))} ATS platforms...")
        all_jobs, boards_ok, boards_failed = await scrape_all(boards)

        # 2b. Scrape remote job boards
        log.info("\nScraping remote job boards...")
        board_jobs = await scrape_all_job_boards()
        all_jobs.extend(board_jobs)

        if not all_jobs:
            log.info("No jobs found across any source.")
            if report_id:
                await finish_scan_report(report_id, boards_scanned=boards_ok,
                                         boards_failed=boards_failed)
            return

        # 3. Filter for CSM/AM roles
        log.info("\nFiltering for CSM/AM roles...")
        csm_jobs = await filter_roles(all_jobs)
        if not csm_jobs:
            log.info("No CSM/AM roles found.")
            if report_id:
                await finish_scan_report(report_id, boards_scanned=boards_ok,
                                         boards_failed=boards_failed,
                                         total_jobs_raw=len(all_jobs))
            return

        # 3b. Enrich descriptions
        log.info("\nFetching descriptions for jobs missing them...")
        csm_jobs = await enrich_descriptions(csm_jobs)

        # 3c. Enrich Greenhouse/Ashby application questions
        log.info("\nEnriching Greenhouse/Ashby application questions...")
        csm_jobs = await enrich_application_questions(csm_jobs)

        # 4. Filter for Africa/Global locations
        log.info("\nFiltering for Africa/Global eligibility...")
        global_jobs, confidences = await filter_locations(csm_jobs)
        if not global_jobs:
            log.info("No global/Africa-eligible CSM/AM roles found.")
            if report_id:
                await finish_scan_report(report_id, boards_scanned=boards_ok,
                                         boards_failed=boards_failed,
                                         total_jobs_raw=len(all_jobs),
                                         csm_roles=len(csm_jobs))
            return

        # 5. Detect visa sponsorship
        for job in global_jobs:
            job["visa_sponsorship"] = detect_visa_sponsorship(job)

        # 6. Push to Supabase
        log.info(f"\nPushing {len(global_jobs)} jobs to Supabase...")
        added = await add_jobs_batch(global_jobs, confidences)

        # 7. Finalize report
        duplicates = len(global_jobs) - added
        if report_id:
            await finish_scan_report(report_id, boards_scanned=boards_ok,
                                     boards_failed=boards_failed,
                                     total_jobs_raw=len(all_jobs),
                                     csm_roles=len(csm_jobs),
                                     global_jobs=len(global_jobs),
                                     new_jobs_added=added, duplicates=duplicates)

        log.info(f"\nDone! {added} new jobs added to Supabase.")
        log.info(f"   Pipeline: {len(all_jobs)} scraped -> {len(csm_jobs)} CSM/AM -> "
                 f"{len(global_jobs)} global -> {added} new")

        # 8. Cleanup stale jobs
        log.info("\nCleaning up stale jobs...")
        await cleanup_stale_jobs(inactive_days=30, delete_days=60)

    except Exception as e:
        log.error(f"Scanner failed: {e}")
        if report_id:
            await finish_scan_report(report_id, status="failed")
        raise
    finally:
        # Always release pooled connections
        await close_client()
        await close_supabase_client()


if __name__ == "__main__":
    asyncio.run(main())
