...
def _err(e: Exception) -> str:
    """str(e) can be empty for some exceptions, and an exception object is
    always truthy, so a naive `e or repr(e)` never actually falls through."""
...
def stream_filter(url: str, output_path: str, countries: set[str] | None,
    ...
    # DIAGNOSTIC (2026): tally the raw country value of every record skipped by the
    # country filter. Country strings are low-cardinality (a few hundred distinct forms
    # at most), so this Counter stays memory-safe even over tens of millions of skipped
    # rows. At the end it shows exactly WHY records were dropped — an empty/missing
    # country field (unrecoverable by normalization) vs. a specific unrecognized
    # code/name (a real normalization gap worth adding to COUNTRY_NORMALIZATION).
    skipped_country_values = Counter()
    raw_row_index = 0
    stopped_early = False
            ...
            for name, domain, row_country in row_source:
                ...
                if countries_lower:
                    if normalized_country not in countries_lower:
                        skipped_country += 1
                        # DIAGNOSTIC: remember the raw (pre-normalization) value so the
                        # end-of-run breakdown can show what forms are actually being dropped.
                        skipped_country_values[row_country_raw] += 1
                        continue
                else:
    ...
    # DIAGNOSTIC breakdown — answers "why did so many get skipped?"
    if skipped_country:
        empty_skips = skipped_country_values.get("", 0)
        log.info(f"Country-field breakdown of the {skipped_country:,} skipped records:")
        log.info(f"  EMPTY/missing country value: {empty_skips:,} "
                 f"({empty_skips / skipped_country * 100:.1f}%) — unrecoverable by normalization")
        non_empty = [(v, n) for v, n in skipped_country_values.most_common(40) if v]
        if non_empty:
            log.info("  top non-empty values that failed to match:")
            for val, n in non_empty:
                log.info(f"    {val!r}: {n:,}")
