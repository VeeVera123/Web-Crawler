...
# 2026-09: a link/slug shaped like ONE SPECIFIC job's own page (e.g.
# "events-marketing-manager-job-description", "executive-director-job-
# description") is not a company's career LISTING page — it's a single
# posting. Both _extract_job_listing_link_candidates (link-follow) and
# _looks_like_real_career_page (the final in-house-candidate gate) need
# to reject these, or a single job's own page gets mistaken for the
# whole company's careers page. Two independent signals, EITHER
# sufficient: (1) an explicit single-posting suffix/trailing numeric
# posting ID, or (2) the path ends in a job-ROLE word (manager/director/
# engineer/...) AND also contains a job-posting-title-shaped word earlier
# in the slug (seniority/department/etc) — a real listings page path is
# generic ("/careers", "/open-positions"), never shaped like one job's
# own title.
_SINGLE_JOB_POSTING_SUFFIX_RE = re.compile(
    r"[-_](job-description|job-details|position-description|job-posting)/?$", re.I)
_TRAILING_NUMERIC_POSTING_ID_RE = re.compile(r"[-_/]\d{4,}/?$")
...
def _looks_like_single_job_posting_path(path: str) -> bool:
    """True when a URL path is shaped like one SPECIFIC job posting's own
    page rather than a company's career LISTING page — e.g.
    "/careers/events-marketing-manager-job-description" or
    "/jobs/executive-director-job-description". See the regexes above
    this function for the two independent signals checked."""
    path = path.rstrip("/")
    if _SINGLE_JOB_POSTING_SUFFIX_RE.search(path) or _TRAILING_NUMERIC_POSTING_ID_RE.search(path):
        return True
    if _JOB_ROLE_ENDING_WORDS_RE.search(path) and _JOB_POSTING_TITLE_WORDS_RE.search(path):
        return True
    return False
...
def _extract_job_listing_link_candidates(html: str, base_url: str) -> list[tuple[str, int]]:
    ...
    try:
        ...
        for a_node in tree.css("a[href]"):
            ...
            # 2026-09: destination-shape check FIRST — a link's TEXT
            # matching _JOB_LISTING_LINK_RE ("view all jobs", etc.) says
            # nothing about where it actually points; without this, a
            # generic-sounding link that happens to point at one specific
            # job's own page (e.g. .../events-marketing-manager-job-
            # description) still scored and got followed as if it were
            # the real listings page.
...
def _looks_like_real_career_page(url: str, text_len: int, has_hiring_vocab: bool, origin: str) -> bool:
    ...
    a generic page at /opportunities), pages that ARE on a known ATS
    vendor's own domain but didn't yield a parseable slug, and — 2026-09 —
    a single job posting's own page (e.g. .../events-marketing-manager-
    job-description) mistaken for the company's career LISTING page: all
    common enough on real sites that skipping these checks would flood
