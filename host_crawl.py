...
async def _robots_allows(session: aiohttp.ClientSession, base_url: str) -> tuple[bool, str]:
    ...
    100K hosts showed 83% "disallowed" — almost certainly mostly dead/slow
    hosts, not real robots.txt blocks, going by discovery.py's own
    documented experience with the same kind of bulk host list — see
    discovery.py's _robots_check_stats comment). Matches discovery.py's
