...
parquet files:    s3://commoncrawl/cc-index/table/cc-main/
                  warc/crawl={crawl}/subset=warc/*.parquet

SPEED OPTIMIZATIONS (2026-09):
1. S3 Protocol: Parquet files are now fetched via s3:// instead of https://.
   This taps into the AWS backbone and leverages S3's highly optimized 
   range-requests via DuckDB's httpfs extension, drastically cutting latency.
2. Query Rewrite: Replaced the heavy GROUP BY aggregation with a simple 
   SELECT DISTINCT filtered on 2xx responses. DuckDB now handles liveness 
   and deduplication natively in SQL.
3. True Parallel Prefetching: Upgraded to a rolling window of 4 parallel 
   DuckDB queries. Each worker gets its own connection to query multiple 
   Parquet files in the background simultaneously.
...
def _get_duckdb_connection():
    ...
    try:
        ...
        # Enable anonymous public access to the commoncrawl S3 bucket
...
def _list_partition_files(crawl_name: str) -> list[str]:
    ...
    try:
        ...
        # Use S3 protocol for faster range-requests via DuckDB's httpfs
        files = [f"s3://commoncrawl/{line.strip()}" for line in lines
