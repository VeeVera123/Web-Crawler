...
# 2026-09: how many files' DuckDB queries run concurrently in the
# background while the crawler works through the current file's hosts
# (see iter_seed_hosts_by_file's "SPEED" docstring). Was a fixed
# max_workers=1 (exactly one file queried ahead) — widened to a small
# bounded window so the query for file N+2, N+3, ... can also be running
# while file N is being crawled, instead of the prefetch always sitting
# one file behind actual demand. Kept modest (not e.g. 8-16) on purpose:
# each in-flight file holds its full un-deduped row list in memory until
# it's consumed (same as before, just now up to this many files' worth
# at once instead of one) — the whole point of this module's per-file
# streaming design was avoiding the 126M-row OOM incident documented in
# the module docstring, so this stays a deliberately small multiple of
# "one file," not an unbounded fan-out. Override via env var to tune per
# runner size.
CC_PREFETCH_WORKERS = int(os.environ.get("CC_PREFETCH_WORKERS", "3"))
...
def _query_file_rows(con, fpath: str, tld_filter: str, shard_clause: str) -> list[tuple]:
    ...
    2026-09: runs on a background prefetch-pool thread (see
    CC_PREFETCH_WORKERS) — pulls a thread-local cursor off the shared
    connection via con.cursor(), DuckDB's own documented pattern for
    running queries from multiple threads concurrently
    (https://duckdb.org/docs/current/guides/python/multiple_threads),
    rather than executing directly on the shared `con` (not safe for
    concurrent use across threads) or opening a brand-new
    duckdb.connect() per call (which would re-run INSTALL/LOAD httpfs and
    every SET option from _get_duckdb_connection on every single file
    query for no benefit — `con` already has all of that configured once,
    up front, and a cursor shares it).
    """
    ...
    """
    return local_con.execute(query).fetchall()
...
def iter_seed_hosts_by_file(partitions: list[str], shard_index: int | None, shard_count: int | None,
    ...
    SPEED (2026-09): up to CC_PREFETCH_WORKERS files' DuckDB queries run
    concurrently in background threads, staying that many files ahead of
    what's actually being crawled — so file N+1's (and N+2's, N+3's, ...)
    query runs WHILE the caller is off crawling file N's hosts (real
    network I/O, seconds), instead of the query and the crawl only ever
    overlapping one file at a time. `.result()` on an already-finished
    future returns instantly, so this only ever helps and never adds
    latency. Files are still consumed and yielded strictly in order (a
    plain FIFO queue of futures, not as_completed) — resume/checkpointing
    depends on file_num meaning "this many files done, in order."
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=CC_PREFETCH_WORKERS) as prefetch_pool:
        for partition_num, crawl_name in enumerate(partitions, start=1):
            ...
            total_hosts = 0
            # Prime the prefetch window with up to CC_PREFETCH_WORKERS
            # files' queries submitted up front, in order — a plain FIFO
            # deque, not as_completed, so the file consumed each loop
            # iteration below is always the one popped from the left,
            # matching the order files are yielded in.
            pending: deque = deque(
                prefetch_pool.submit(_query_file_rows, con, files[w], tld_filter, shard_clause)
                for w in range(min(CC_PREFETCH_WORKERS, len(files)))
            )
            ...
            for file_num, fpath in enumerate(files, start=this_partition_start + 1):
                idx = file_num - this_partition_start - 1  # 0-based index into `files`, matches `pending`'s order
                rows_future = pending.popleft()
                try:
                    rows = rows_future.result(timeout=query_timeout)
                    ...
                    rows = []
                # Keep the prefetch window full: submit the file
                # CC_PREFETCH_WORKERS ahead of the one just consumed, so
                # up to that many queries stay running in the background
                # while this file's hosts get crawled below.
                next_i = idx + CC_PREFETCH_WORKERS  # index into the (sliced) `files` list
                if next_i < len(files):
                    pending.append(prefetch_pool.submit(_query_file_rows, con, files[next_i], tld_filter, shard_clause))
