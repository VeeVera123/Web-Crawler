"""
HOST CRAWL V2 — a thin, disposable seed source on top of node.py (the
permanent engine). This file's only job: stream candidate hostnames out
of Common Crawl's Host Index (via DuckDB, one Parquet file at a time —
see STREAMING below) and hand them to node.crawl_batch(). All
fetch/parse/detect/write logic lives in node.py.
...
Usage:
    python host_crawl_v2.py                                        # 1 partition, unsharded (dev only)
    python host_crawl_v2.py --crawl CC-MAIN-2025-18 --partitions 3  # 3 contiguous partitions
    python host_crawl_v2.py --crawl-list CC-MAIN-2025-18,CC-MAIN-2024-42
    python host_crawl_v2.py --shard-index 0 --shard-count 10        # production shape
"""
import argparse
import asyncio
