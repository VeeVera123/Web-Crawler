...
from config_test import LLM_PROVIDER, LLM_API_KEY, LLM_MODEL, LLM_BASE_URL, AI_BATCH_SIZE, AI_PARALLEL_REQUESTS
...
def _classify_location_batch(batch_jobs: list[dict], max_user_chars: int) -> list[str]:
    ...
    log.info(f"  [AI] Sending batch of {len(batch_jobs)} to LLM (desc cap: {max_desc} chars)")
