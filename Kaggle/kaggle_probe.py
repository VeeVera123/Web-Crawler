...
permanent engine), same role opendata_probe.py plays for the OpenData
dataset and kaggle_probe.py's PDL seed plays for People Data Labs.

Copied directly from opendata_probe.py (2026-08) since the shape is
...
Reads the FILTERED, already-small CSV kaggle_seed.py produced (and
kaggle.yml's workflow downloaded/joined from GitHub Release assets, if
it was split across multiple <2GB chunks — see kaggle_seed.py's module
docstring for the full download->filter->upload->rejoin pipeline). Column
names are fixed ("name","domain","country") since kaggle_seed.py always
writes that exact header regardless of what the raw upstream source called
its own columns — no alias-detection needed here.

Usage:
    pip install aiohttp aiodns selectolax python-dotenv
    python kaggle_probe.py
    python kaggle_probe.py --shard-index 0 --shard-count 15
"""
...
log = logging.getLogger("kaggle_probe")

KAGGLE_FILTERED_PATH = os.environ.get("KAGGLE_FILTERED_PATH", "kaggle_companies_filtered.csv")
KAGGLE_ROW_LIMIT = int(os.environ.get("KAGGLE_ROW_LIMIT", "0"))  # 0 = no cap

# Same {name,domain,country} shape opendata_probe.py/github_org_seed.py
...
SEED_SOURCE_LABEL = os.environ.get("SEED_SOURCE_LABEL", "kaggle_probe")

# kaggle_seed.py always writes exactly this header — no alias detection
# needed (contrast kaggle_probe.py reading PDL's own raw file, which has
# to tolerate 'domain' vs 'website' etc.).
NAME_COL, DOMAIN_COL, COUNTRY_COL = "name", "domain", "country"

# Kept identical to opendata_probe.py's/opendata_seed.py's DEFAULT_COUNTRIES
# deliberately — kaggle_seed.py already filtered to this set upstream,
# so this second pass is normally a no-op; it stays here as a safety net
# in case KAGGLE_FILTERED_PATH ever points at an unfiltered/differently
# filtered file, and so --country/--all-countries work the same way
# opendata_probe.py's do if someone wants a narrower crawl than the seed.
DEFAULT_COUNTRIES = {
...
def read_seed_csv(limit: int = KAGGLE_ROW_LIMIT, countries: set[str] | None = None,
                  shard_index: int | None = None, shard_count: int | None = None) -> list[dict]:
    """Single streaming pass over kaggle_seed.py's filtered CSV — same
    shard-aware inline modulo filter as opendata_probe.py's read_seed_csv
    ...
    crashing."""
    if not os.path.exists(KAGGLE_FILTERED_PATH):
        log.warning(f"Filtered seed CSV not found at '{KAGGLE_FILTERED_PATH}' — nothing to crawl. "
                    f"Run kaggle_seed.py (and download/join its Release asset chunks) first.")
        return []
    ...
    try:
        with open(KAGGLE_FILTERED_PATH, newline="", encoding="utf-8", errors="ignore",
...
def main():
    parser = argparse.ArgumentParser(description="Kaggle domain-crawl discovery (async)")
