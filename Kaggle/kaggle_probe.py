...
No separate kaggle_seed.py: the seed data itself is a static manual
download (PDL's download form is email/use-case gated, not a scriptable
URL), not something generated by a script.

DATASET UPGRADE (2026-08): was the 7M-row Kaggle mirror, now PDL's own
current free dataset (22M+ companies, download directly from
peopledatalabs.com/company-dataset — CC BY 4.0). Column names differ
between the two ('domain' on the old Kaggle mirror vs 'website' on PDL's
own current file) — read_seed_csv() below auto-detects whichever is
present so either file works without editing this script. The extra
columns the new file has (industry, size, linkedin_url, founded) are
read but unused — node.crawl_batch() only needs a domain list; nothing
here throws them away destructively, they're just not in the CSV's
header set it looks for.
...
Usage:
    ...
    python kaggle_probe.py --shard-index 0 --shard-count 20
"""
import argparse
import asyncio
import csv
...
# Column aliases: old 7M Kaggle mirror used 'domain'; PDL's own current
# free-dataset download uses 'website'. Whichever is present wins — first
# match in each tuple, case-insensitive header match.
...
def read_seed_csv(limit: int = PDL_ROW_LIMIT, countries: set[str] | None = None,
    ...
    build. Applies country + domain-validity filtering, THEN (only if
    ...
    log.info(f"Seed dataset: {total_rows:,} total rows{filter_note}, {kept_before_shard:,} with a usable domain "
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Kaggle/PDL domain-crawl discovery (async)")
