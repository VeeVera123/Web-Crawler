"""
Two-stage classifier — ANTHROPIC VARIANT
Swap this into classifier.py to use Anthropic Claude for classification.

  Stage 1 — keyword filter for CSM/AM role titles (fast, no API)
  Stage 2 — Anthropic Claude for ambiguous titles (batched)

Then a separate location filter:
  Stage 3 — keyword check for Africa/Global locations
  Stage 4 — Anthropic Claude for ambiguous locations + description scanning
"""
...
import logging
import anthropic
from config_anthropic import ANTHROPIC_API_KEY, ANTHROPIC_MODEL, AI_BATCH_SIZE
...
def _ai_call_with_retry(system: str, user_msg: str, max_tokens: int = 500) -> str | None:
    """Call Anthropic Claude with prompt caching + retry on rate limit.
    The system prompt is cached (5 min TTL) so repeated calls only pay 10% input cost.
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if attempt < MAX_RETRIES - 1:
                ...
                log.warning(f"Anthropic rate limit hit, retrying in {delay}s (attempt {attempt + 1}/{MAX_RETRIES})")
                time.sleep(delay)
                continue
            else:
...
def ai_classify_roles(titles: list[str]) -> dict[str, bool]:
    """Send ambiguous titles to Anthropic Claude. Returns {title: is_relevant}.
    ...
    for i in range(0, len(titles), AI_BATCH_SIZE):
        ...
        text = _ai_call_with_retry(ROLE_SYSTEM_PROMPT, user_msg, max_tokens=500)
...
def ai_classify_locations(jobs: list[dict]) -> list[str]:
    """
    Send ambiguous jobs to Anthropic Claude for location classification.
    ...
    for i in range(0, len(jobs), AI_BATCH_SIZE):
        ...
        text = _ai_call_with_retry(LOCATION_SYSTEM_PROMPT, user_msg, max_tokens=1000)
