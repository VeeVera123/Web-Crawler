"""
CTLOGS SEED (renamed from ctlog_extract.py, 2026-08) — the old standalone
ctlogs_probe.py / ctlogs_icims_diagnostic.py / ctlogs_probe_direct.py are
all retired; this is the one production version. Now depends on node.py
for shared plumbing (Supabase writes, USER_AGENT/timeout, connector) —
its own crt.sh/live-resolve logic stays here since node.py's crawl_batch
is a different shape of problem (it visits a COMPANY's site looking for
outbound ATS links; this script queries crt.sh directly and, for Class B
platforms, fetches the ATS TENANT host itself).
...
script (ctlogs_probe.py) per explicit direction, so a slow or flaky
verification pass never blocks or reruns extraction, and vice versa.
...
Usage:
    ...
    python ctlogs_seed.py --platform taleo
    python ctlogs_seed.py --platform oracle_cloud_hcm
    python ctlogs_seed.py --platform successfactors
"""
import argparse
import asyncio
import logging
...
logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)-8s %(message)s",
                     datefmt="%H:%M:%S")
log = logging.getLogger("ctlogs_seed")
...
# REQUEST_TIMEOUT/USER_AGENT come from node.py now — one place to tune them.
...
async def fetch_existing_slug_registry_slugs(session: aiohttp.ClientSession, ats: str) -> set[str]:
    ...
    while True:
        ...
        try:
            async with session.get(
                f"{node.SUPABASE_URL}/rest/v1/slug_registry",
                headers=headers,
...
async def write_to_staging_table(session: aiohttp.ClientSession, ats: str, root_domain: str,
    ...
    # discovery_method matches this file's own name — the actual write goes
    # through node.write_rows_to_staging_table(), same retried/chunked
    # upsert every other seed source uses, instead of a second copy here.
    rows = [
        {"ats": ats, "slug": slug, "source_hostname": host, "root_domain": root_domain,
         "discovery_method": "certificate_transparency_seed"}
        for slug, host in slugs.items()
    ]
    written = await node.write_rows_to_staging_table(session, rows)
